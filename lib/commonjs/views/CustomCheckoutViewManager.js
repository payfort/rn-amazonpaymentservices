...
const ViewManager = (0, _reactNative.requireNativeComponent)('CustomCheckout');
...
const CustomCheckoutViewManager = props => {
  ...
  const DEFAULT_STYLE = {
    width: _styleDefaults.CUSTOM_UI_VIEW_WIDTH,
    height: _styleDefaults.CUSTOM_UI_VIEW_HEIGHT
  };
  ...
  // @ts-ignore
  _reactNative.UIManager.CustomCheckout.Commands.create.toString(), [viewId]);
  ...
  const setUpScreen = () => {
    ...
    !isIOS && _RnAmazonPaymentServiceSdk.default.callCustomFortRequest(requestObject, payButtonProps, environment, returnSuccessParsed, returnFailureParsed);
  };
  const styleAndroid = {
    style: {
      // @ts-ignore
...
var _default = exports.default = CustomCheckoutViewManager;
